    function w = pol_ridge_fit(x, t, M, lambda, opt)
    %INITIALISATION
    ...
    elseif opt.meth == 2 
        ...
        while (sens>opt.tol & ii < opt.nitermax)
            wprev = w;
            grad = (phi')*(phi*w-t)+lambda*w;%FORMULA GOT IN THE WASHINGTON UNIVERSITY COURSE
            w = w - gamma*grad;
            sens = norm(w - wprev)/norm(w);
            ii = ii + 1;
            lossfunct(ii) = 0.5 * norm(phi*w-t);
            if opt.verb == 1 %VISUALISATION DE LA FONCTION D'ERREUR 
